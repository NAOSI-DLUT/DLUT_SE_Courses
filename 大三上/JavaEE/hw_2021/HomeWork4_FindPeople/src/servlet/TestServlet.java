...
public class TestServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getAttribute("information") !=null){
			query = (String) request.getAttribute("information");
		}else{
			query = request.getParameter("information");
		}
...
//		ReadFile rf = new ReadFile(fileName, cache);
		...
		if (query.length() == 0) {//没有信息转发到原网页
			...
			ArrayList<Man> manList = (ArrayList<Man>) bd.searchOf(query,1,5);

			HttpSession hs=request.getSession();
			hs.setAttribute("role","none");
